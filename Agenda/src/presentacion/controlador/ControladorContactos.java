...
public class ControladorContactos implements ActionListener, Observador {
	...
	private boolean camposSonValidos() {
		...
		if (algunCampoEstaVacio()) {
			JOptionPane.showMessageDialog(formularioContactos,
					"No se pudo editar/agregar el contacto. Algñn campo esta vacio", "Error de contacto",
					JOptionPane.ERROR_MESSAGE);
			return false;
		} else {
			...
			if (!sonValidos) {
				JOptionPane.showMessageDialog(formularioContactos,
						"Error de validaciñn, verifica que los campos esten ingresados correctamente.",
